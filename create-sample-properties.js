// Script to create sample properties in Supabase database
// This script uses the properties.createProperty() function from lib/properties.ts
...
async function createSampleProperties() {
  ...
  for (let i = 0; i < sampleProperties.length; i++) {
    ...
    try {
      const { data, error } = await properties.createProperty(property);
      ...
      if (error) {
        ...
        console.log(`   Price: $${property.asking_price.toLocaleString()}`);
        console.log(`   Location: ${property.city}, ${property.state}`);
        console.log('');
      }
    } catch (err) {
      console.error(`Failed to create property ${i + 1}:`, err);
    }
  }
  
  console.log('Sample property creation complete!');
}

// Execute the function
createSampleProperties().catch(console.error);

export { sampleProperties, createSampleProperties };